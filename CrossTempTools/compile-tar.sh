...
# Search for files whose name matches "tar*"
# and save such into the pkg variable.
cd $LFS/sources
pkg=$(find . -maxdepth 1 -type f -name "tar*" | head -n 1)

if [ -z "$pkg" ]; then
    echo "Error: No directories matching 'tar*' found"
else
    tar -xvf $pkg
    dir=$(find . -maxdepth 1 -type d -name "tar*" | head -n 1)
    cd $dir
